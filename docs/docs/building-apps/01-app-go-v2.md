...
1. Create the `AppConfig` variable:
    ...
    ```go reference
    https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/simapp/app_config.go#L91-L93
    ```

2. Configure the `runtime` module:

    ```go reference
    https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/simapp/app_config.go#L94-L158
    ```

3. Configure the modules defined in the `BeginBlocker` and `EndBlocker` and the `tx` module:

    ```go reference
    https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/simapp/app_config.go#L159-L177
    ```

    ```go reference
    https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/simapp/app_config.go#L192-L194
    ```

### Complete `app_config.go`

```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/simapp/app_config.go#L52-L254
```
...
A more complete example of `app.yaml` can be found [here](https://github.com/lightmos/lightmos-sdk/blob/91b1d83f1339e235a1dfa929ecc00084101a19e3/simapp/app.yaml).
...
In short `depinject` and the [`runtime` package](https://pkg.go.dev/github.com/lightmos/lightmos-sdk/runtime) abstract the wiring of the app, and the `AppBuilder` is the place where the app is constructed. [`runtime`](https://pkg.go.dev/github.com/lightmos/lightmos-sdk/runtime) takes care of registering the codecs, KV store, subspaces and instantiating `baseapp`.

```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/simapp/app_v2.go#L158-L291
```
...
```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/simapp/app_v2.go#L186-L216
```
...
```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/simapp/app_v2.go#L75-L395
```
