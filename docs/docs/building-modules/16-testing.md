...
```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/scripts/mockgen.sh#L3-L6
```
...
As an example, we will walkthrough the [keeper tests](https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/x/gov/keeper/keeper_test.go) of the `x/gov` module.

The `x/gov` module has a `Keeper` type requires a few external dependencies (ie. imports outside `x/gov` to work properly).

```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/x/gov/keeper/keeper.go#L61-L65
```
...
```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/x/gov/keeper/common_test.go#L67-L81
```

This allows us to test the `x/gov` module without having to import other modules.

```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/x/gov/keeper/keeper_test.go#L3-L35
```

We can test then create unit tests using the newly created `Keeper` instance.

```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/x/gov/keeper/keeper_test.go#L73-L91
```
...
In the SDK, we locate our integration tests under [`/tests/integrations`](https://github.com/lightmos/lightmos-sdk/tree/main/tests/integration).

The goal of these integration tests is to test a component with a minimal application (i.e. not `simapp`). The minimal application is defined with the help of [`depinject`](../tooling/02-depinject.md) – the SDK dependency injection framework, and includes all necessary modules to test the component. With the helps of the SDK testing package, we can easily create a minimal application and start the application with a set of genesis transactions: <https://github.com/lightmos/lightmos-sdk/blob/main/testutil/sims/app_helpers.go>.
...
For creating a minimal application, we use [`simtestutil.Setup`](https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/testutil/sims/app_helpers.go#L95-L99) and an [`AppConfig`](../tooling/02-depinject.md) of the `x/distribution` minimal dependencies.

For instance, the `AppConfig` of `x/distribution` is defined as:

* https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/x/distribution/testutil/app_config.go

This is a stripped down version of the `simapp` `AppConfig`:

* https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/simapp/app_config.go

:::note
You can as well use the `AppConfig` `configurator` for creating an `AppConfig` [inline](https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/x/slashing/app_test.go#L54-L62). There no difference between those two ways, use whichever you prefer.
:::

```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/tests/integration/distribution/keeper/keeper_test.go#L28-L33
```

Now the types are injected and we can use them for our tests:

```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/tests/integration/distribution/keeper/keeper_test.go#L21-L53
```
...
```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/tests/integration/bank/keeper/deterministic_test.go#L102-L115
```
...
```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/x/gov/simulation/operations_test.go#L292-L310
```

```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/x/gov/simulation/operations_test.go#L69-L111
```

## End-to-end Tests

End-to-end tests are at the top of the [test pyramid](https://martinfowler.com/articles/practical-test-pyramid.html).
They must test the whole application flow, from the user perspective (for instance, CLI tests). They are located under [`/tests/e2e`](https://github.com/lightmos/lightmos-sdk/tree/main/tests/e2e).

For that, the SDK is using `simapp` but you should use your own application (`appd`).
Here are some examples:

* SDK E2E tests: <https://github.com/lightmos/lightmos-sdk/tree/main/tests/e2e>.
