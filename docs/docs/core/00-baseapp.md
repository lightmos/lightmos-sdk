...
```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/baseapp/baseapp.go#L50-L146
```
...
* [`TxDecoder`](https://pkg.go.dev/github.com/lightmos/lightmos-sdk/types#TxDecoder): It is used to decode
...
possibility to provide additional [`options`](https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/baseapp/options.go)
...
The [default `msgServiceRouter` included in `BaseApp`](https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/baseapp/msg_service_router.go) is stateless. However, some applications may want to make use of more stateful routing mechanisms such as allowing governance to disable certain routes or point them to new modules for upgrade purposes. For this reason, the `sdk.Context` is also passed into each [route handler inside `msgServiceRouter`](https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/baseapp/msg_service_router.go#L31-L32). For a stateless router that doesn't want to make use of this, you can just ignore the `ctx`.
...
```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/x/auth/ante/basic.go#L96
```
...
```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/baseapp/baseapp.go#L663-L672
```
...
```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/types/handler.go#L6-L8
```
...
`BaseApp` holds an `anteHandler` as parameter that is initialized in the [application's constructor](../basics/00-app-anatomy.md#application-constructor). The most widely used `anteHandler` is the [`auth` module](https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/x/auth/ante/ante.go).
...
```go reference
https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/x/auth/posthandler/post.go#L1-L15
```
...
* Initialize [`deliverState`](#state-updates) with the latest header using the `req abci.RequestBeginBlock` passed as parameter via the `setState` function.
  ...
  ```go reference
  https://github.com/lightmos/lightmos-sdk/blob/v0.47.0-rc1/baseapp/baseapp.go#L406-L433
  ```
