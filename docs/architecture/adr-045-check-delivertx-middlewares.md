...
* 07.12.2021: Update `tx.Handler` interface ([\#10693](https://github.com/lightmos/lightmos-sdk/pull/10693)).
* 17.05.2022: ADR is abandoned, as middlewares are deemed too hard to reason about.

## Status

ABANDONED. Replacement is being discussed in [#11955](https://github.com/lightmos/lightmos-sdk/issues/11955).
...
BaseApp's implementation of ABCI `{Check,Deliver}Tx()` and its own `Simulate()` method call the `runTx` method under the hood, which first runs antehandlers, then executes `Msg`s. However, the [transaction Tips](https://github.com/lightmos/lightmos-sdk/issues/9406) and [refunding unused gas](https://github.com/lightmos/lightmos-sdk/issues/2150) use cases require custom logic to be run after the `Msg`s execution. There is currently no way to achieve this.

An naive solution would be to add post-`Msg` hooks to BaseApp. However, the Cosmos SDK team thinks in parallel about the bigger picture of making app wiring simpler ([#9181](https://github.com/lightmos/lightmos-sdk/discussions/9182)), which includes making BaseApp more lightweight and modular.
...
* Allow custom logic to be run before an after `Msg` execution. This enables the [tips](https://github.com/lightmos/lightmos-sdk/issues/9406) and [gas refund](https://github.com/lightmos/lightmos-sdk/issues/2150) uses cases, and possibly other ones.
...
* [#9934](https://github.com/lightmos/lightmos-sdk/discussions/9934) Decomposing BaseApp's other ABCI methods into middlewares.
...
* Initial discussion: https://github.com/lightmos/lightmos-sdk/issues/9585
* Implementation: [#9920 BaseApp refactor](https://github.com/lightmos/lightmos-sdk/pull/9920) and [#10028 Antehandlers migration](https://github.com/lightmos/lightmos-sdk/pull/10028)
